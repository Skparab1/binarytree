...
function drawspaces(leftmargin,arr){

    // create that many spaces
    let thedisp = document.getElementById("feedbackspacer");
    ...
    thedisp.innerHTML += `<div class='spacebox' style='margin-left: ${leftmargin}%; background-color: ${clr}'>${arr[0]}</div>`;
    ...
    while (i < arr.length){
        ...
        thedisp.innerHTML += `<div class='spacebox' style="background-color: ${clr}">${arr[i]}</div>`;
        i += 1;
    }
}
...
function lightup(){
    // erase the old light up
    redrawtree();

    // create the display thing

    let numnodes = countnodes_ignorenull(root);
    spaceon = 0;
    animarr = [];
    
    //calculate margin left
    let taken = 3*numnodes;
    let remaining = 100-taken;
    ...
    drawspaces(leftmargin, animarr);

    if (setorder == "preorder"){
        glowtreepreorder(root);
    } else if (setorder == "inorder"){
        glowtreeinorder(root);
    } else {
        glowtreepostorder(root);
    }
}
...
async function glowtreepreorder(start){
    if (start == null || start.value == "deleted"){
        return;
    }
...
// counts the nodes ignoring null
function countnodes_ignorenull(root){
    if (root == null || root.value == "deleted"){
        return 0;
    }
    return (1 + countnodes_ignorenull(root.getleft) + countnodes_ignorenull(root.getright));
}
...
function randinbetween(min, max){
    // BOTH ARE INCLUSIVE
    let range = max-min+1;
...
// this is to generate an exercise for the user
function generateexercise(){
    ...
    useorder = orders[rand];

    let rand2 = Math.floor(Math.random()*2);
    uselevel = levels[rand2];

    // now figure out the number of numbers/letters that we can have
    // this has to be less than the maximum amount of nodes that a tree with uselevel levels can support
    // and more than the minumum nodes it can support

    let minallowednodes = Math.pow(2, (uselevel-1));
    let maxallowednodes = Math.pow(2, (uselevel))-1;

    // the prompt should be given those many numbers
    let numberofvalues = randinbetween(minallowednodes,maxallowednodes);

    // decide whether to make it alphabetical or numeric
    // and get that array
    let valuearray = [];
    ...
    if (rand3 == 0){
        // numeric
        valuearray = genlist(numberofvalues);
    } else {
        // alphabetical
        valuearray = genlistabc(numberofvalues);
    }
    ...
    return `Create a <span style="color: var(--contrast); font-weight: bold;">${uselevel}</span> level binary tree that outputs <span style="color: var(--contrast); font-weight: bold;">${valuearray.toString()}</span> in <span style="color: var(--contrast); font-weight: bold;">${useorder}</span> traversal.`;
}

function checklevels(root, clevel){
    if (root == null || root.value == "deleted"){
        return clevel;
    }
    ...
    return Math.max(clevel, checklevels(root.getleft, clevel), checklevels(root.getright, clevel));
}

function verifyexercise(){
    let getsolution;
    if (setorder == "preorder"){
        getsolution = traversepreorder(root).replaceAll('deleted','');
    } else if (setorder == "inorder"){
        getsolution = traverseinorder(root).replaceAll('deleted','');
    } else {
        getsolution = traversepostorder(root).replaceAll('deleted','');
    }

    let reallevels = checklevels(root, 0);

    // alert(reallevels,setlevels);

    if (String(getsolution.replaceAll(" ","").toUpperCase()) === String(setarray.toString().replaceAll(",",""))){
        if (reallevels == setlevels){
            // alert("correct");
            openel("corr");
        } else {
            // alert("incorrect number of levels");
            document.getElementById("explanation").innerHTML = "Expected levels: "+setlevels+"<br> Your solution levels: "+reallevels;
            openel("wr");
        }
    } else {
        let feedbackres = getsolution.replaceAll(" ",",");

        while (feedbackres.includes(",,")){
            feedbackres = feedbackres.replaceAll(",,",",");
        }

        if (feedbackres.substring(1) == ","){
            feedbackres = feedbackres.substring(1);
        }
        
        document.getElementById("explanation").innerHTML = "Expected: "+setarray.toString()+"<br> Your solution: "+feedbackres.toUpperCase();
        openel("wr");
    }
}
